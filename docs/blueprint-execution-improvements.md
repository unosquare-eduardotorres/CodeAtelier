...
| A4 | OpenCode usage telemetry | M | – | 0 | ☐ | **Prerequisite** — OpenCode is unmeasurable today |
| E11 | Attempt-level telemetry + stall/nudge incidents | M | – | 0 | ☐ | Makes every §0 metric queryable |
...
- **Measure:** prefix floor = `MIN(turn_usage.context_tokens)` for `blueprint-build` conversations (baseline **103 K**); average `context_tokens` (baseline **167 K**); cumulative ctx per attempt. Target: prefix floor under 65 K. Unit-level: `blueprint-prefix-size.test.ts` ratchets the assembled BUILD prompt in chars.
