...
test.describe('Health Overview & Sidebar', () => {
  ...
  test('track sidebar renders with all available audit tracks', async ({ electronPage: page }) => {
    ...
    const hasLabel = await auditorsLabel.first().isVisible({ timeout: 3_000 }).catch(() => false)
  ...
  test('overview dashboard renders aggregate scores when no track selected', async ({ electronPage: page }) => {
    const ready = await ensureWorkspaceReady(page)
    if (!ready) { test.skip(); return }
    ...
    if (!hasOverview) { test.skip(); return }
    ...
    const hasCompletion = await completionText.first().isVisible({ timeout: 3_000 }).catch(() => false)
    expect(typeof hasCompletion).toBe('boolean')
  })

  test('overview shows individual track summary cards with scores', async ({ electronPage: page }) => {
    const ready = await ensureWorkspaceReady(page)
    if (!ready) { test.skip(); return }
  ...
  test('clicking a summary card in overview navigates to detail panel', async ({ electronPage: page }) => {
    const ready = await ensureWorkspaceReady(page)
    if (!ready) { test.skip(); return }
    ...
    if (hasSidebar) {
      ...
      const hasActive = await activeTrack.first().isVisible({ timeout: 3_000 }).catch(() => false)
