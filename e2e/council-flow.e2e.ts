...
test.describe('Council Flow', () => {
  ...
  async function navigateToCouncil(page: import('@playwright/test').Page): Promise<boolean> {
    ...
    const councilTab = page.locator('button').filter({ hasText: /council/i }).first()
  ...
  test('session history lists past council sessions', async ({ electronPage: page }) => {
    ...
    const hasFilter = await filterBar.first().isVisible({ timeout: 3_000 }).catch(() => false)

    // Either filter bar (history exists) or empty state CTA
    const hasCta = await page.locator('[data-testid="council-start-btn"]').isVisible({ timeout: 2_000 }).catch(() => false)
