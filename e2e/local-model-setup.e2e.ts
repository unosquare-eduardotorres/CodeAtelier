...
test.describe('Local Model Setup', () => {
  ...
  async function navigateToModelsSettings(
    ...
    const modelsTab = page.locator('button, [role="tab"]').filter({ hasText: /^[\s]*Models[\s]*$/ }).first()
  ...
  test('test connection button validates endpoint', async ({ electronPage: page }) => {
    ...
    const ollamaModal = page.locator('[data-testid="ollama-setup-modal"]')
    if (!(await ollamaModal.isVisible({ timeout: 3_000 }).catch(() => false))) { test.skip(); return }

    const testBtn = page.locator('[data-testid="ollama-test-btn"]')
    if (!(await testBtn.isVisible({ timeout: 2_000 }).catch(() => false))) { test.skip(); return }

    await testBtn.click()
    await page.waitForTimeout(2_000)

    const hasSuccess = await ollamaModal.getByText(/ready|connected|running/i).first()
      .isVisible({ timeout: 5_000 }).catch(() => false)
    const hasError = await ollamaModal.getByText(/not found|not running|error|cannot reach/i).first()
  ...
  test('embedding model setup modal renders', async ({ electronPage: page }) => {
    ...
    if (!hasEmbedding) {
      ...
      if (await settingsBtn.isVisible({ timeout: 2_000 }).catch(() => false)) {
        await settingsBtn.click()
        await page.waitForTimeout(1_000)
        const codeIntelTab = page.locator('button, [role="tab"]').filter({ hasText: /code intelligence|embedding/i }).first()
  ...
  test('embedding model download shows progress', async ({ electronPage: page }) => {
    ...
    const embeddingModal = page.locator('[data-testid="embedding-setup-modal"]')
    if (!(await embeddingModal.isVisible({ timeout: 3_000 }).catch(() => false))) { test.skip(); return }
    ...
    if (await downloadBtn.isVisible({ timeout: 2_000 }).catch(() => false)) {
      ...
      await page.waitForTimeout(2_000)
      const hasProgress = await embeddingModal.getByText(/%/).first()
        .isVisible({ timeout: 5_000 }).catch(() => false)
      const hasReady = await embeddingModal.getByText(/model ready/i)
        .isVisible({ timeout: 5_000 }).catch(() => false)
      expect(hasProgress || hasReady).toBeTruthy()
    } else {
      const isReady = await embeddingModal.getByText(/model ready/i)
