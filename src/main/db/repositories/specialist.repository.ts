...
export class SpecialistRepository {
  findAll(): Specialist[] {
    ...
    const rows = db.prepare('SELECT * FROM specialists ORDER BY priority ASC').all() as SpecialistRow[];
    return rows.map(mapRow);
  }

  findById(id: string): Specialist | undefined {
    const db = getDatabase();
    const row = db.prepare('SELECT * FROM specialists WHERE id = ?').get(id) as SpecialistRow | undefined;
    return row ? mapRow(row) : undefined;
  }

  findByAgentId(agentId: string): Specialist | undefined {
    const db = getDatabase();
    const row = db.prepare('SELECT * FROM specialists WHERE agent_id = ?').get(agentId) as SpecialistRow | undefined;
    return row ? mapRow(row) : undefined;
  }

  findActive(): Specialist[] {
    const db = getDatabase();
    const rows = db.prepare('SELECT * FROM specialists WHERE is_active = 1 ORDER BY priority ASC').all() as SpecialistRow[];
    return rows.map(mapRow);
  }
  ...
  update(id: string, data: UpdateSpecialistInput): Specialist {
    ...
    if (data.sourceYaml !== undefined) { sets.push('source_yaml = ?'); values.push(data.sourceYaml); }
  ...
  removeSkill(specialistId: string, skillId: string): void {
    const db = getDatabase();
    db.prepare('DELETE FROM specialist_skills WHERE specialist_id = ? AND skill_id = ?')
      .run(specialistId, skillId);
  }
  ...
  canDelete(id: string): { allowed: boolean; blockingSkills?: string[] } {
    ...
    const blockingSkills = db.prepare(`
