...
export function registerCheckpointIpc(): void {
  ...
  ipcMain.handle(
    ...
    (event, rawArgs: unknown) => {
      ...
      const checkpointId = requireString(args, 'checkpointId', IPC_CHANNELS.CHECKPOINT_APPROVAL_RESPONSE)
      const approved = optionalBoolean(args, 'approved', IPC_CHANNELS.CHECKPOINT_APPROVAL_RESPONSE) ?? false
