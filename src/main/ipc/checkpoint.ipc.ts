...
export function registerCheckpointIpc(): void {
  ipcMain.handle(IPC_CHANNELS.CHECKPOINT_LIST, (event, args: { conversationId: string }) => {
  ...
  ipcMain.handle(IPC_CHANNELS.CHECKPOINT_RESTORE, (event, args: { checkpointId: string }) => {
    validateSender(event)
