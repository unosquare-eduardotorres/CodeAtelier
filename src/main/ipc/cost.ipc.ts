...
export function registerCostIpc(): void {
  ipcMain.handle(IPC_CHANNELS.COST_GET_WORKSPACE_SUMMARY, (event, rawArgs: unknown) => {
  ...
  ipcMain.handle(IPC_CHANNELS.COST_GET_WORKSPACE_CONVERSATIONS, (event, rawArgs: unknown) => {
    validateSender(event)
