...
if (auditLoaded) {
  describe('audit.ipc — channel registration (deep)', () => {
    const auditCh = [...getHandlers().keys()].filter((c) => c.startsWith('audit:'))
  ...
  describe('audit.ipc — argument validation (deep)', () => {
    ...
    const startCh = [...getHandlers().keys()].find(
      (c) =>
        c.startsWith('audit:') &&
        (c.includes('start') || c.includes('Start') || c.includes('run') || c.includes('Run'))
    )
    if (startCh) {
    ...
    // audit:deleteRun
    if (getHandlers().has('audit:deleteRun')) {
    ...
    // audit:getPlans
    if (getHandlers().has('audit:getPlans')) {
    ...
    // audit:generatePlan
    if (getHandlers().has('audit:generatePlan')) {
  ...
  describe('audit.ipc — handler bodies (deep)', () => {
    ...
    // Specific body tests
    if (getHandlers().has('audit:cancel')) {
