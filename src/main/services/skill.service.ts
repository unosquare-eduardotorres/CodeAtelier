...
import { readFileSync, writeFileSync, copyFileSync, unlinkSync, renameSync, statSync, existsSync } from 'node:fs';
...
export class SkillService {
  ...
  validateSkillFile(filePath: string): { valid: boolean; error?: string; lastUpdated?: string } {
    ...
    if (!dateMatch) {
      return {
        valid: false,
        error: 'Missing required "Last updated: YYYY-MM-DD" field. Skill files must include this date for staleness tracking.'
      };
    }
  ...
  async importSkill(sourcePath: string): Promise<Skill> {
    ...
    try {
      ...
      if (fileCopied && existsSync(targetPath)) {
        try { unlinkSync(targetPath); } catch { /* ignore cleanup error */ }
      }
      if (skill) {
        try { skillRepository.delete(skill.id); } catch { /* ignore cleanup error */ }
      }
      throw error;
    }
  }
  ...
  async activateSkill(id: string): Promise<Skill> {
    ...
    try {
      ...
      try { skillRepository.setActive(id, false); } catch { /* ignore */ }
      throw error;
    }
  }
  ...
  async deactivateSkill(id: string): Promise<Skill> {
    ...
    try {
      ...
      try { skillRepository.setActive(id, true); } catch { /* ignore */ }
      throw error;
    }
  }

  // ── Opus CLAUDE.md Update ──
  private async updateClaudeMd(skillFilePath: string, operation: 'activate' | 'deactivate'): Promise<void> {
    ...
    const prompt = operation === 'activate'
...
4. Keep all other content intact`;
  ...
  private spawnOpusCall(prompt: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      skillLogger.info(`Opus CLAUDE.md update spawned (no timeout, prompt length: ${prompt.length} chars)`);
      ...
      child.on('exit', (code) => {
        ...
        skillLogger.info(`Opus exited with code ${code} (stdout: ${stdout.length} chars, stderr: ${stderr.length} chars)`);
