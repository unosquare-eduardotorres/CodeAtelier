...
describe('createCodeGraphFirstHook — warn mode', () => {
  test('allows graph tool as first call', async () => {
    ...
    const result = await hook(makeInput('mcp__code-graph__search_identifiers'))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read after a graph tool has been called', async () => {
    const hook = createCodeGraphFirstHook('warn')
    await hook(makeInput('graph_map'))
    const result = await hook(makeInput('Read', { file_path: '/proj/src/index.ts' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Grep after a graph tool has been called', async () => {
    const hook = createCodeGraphFirstHook('warn')
    await hook(makeInput('search_identifiers'))
    const result = await hook(makeInput('Grep', { path: '/proj/src', pattern: 'foo' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Glob after a graph tool has been called', async () => {
    const hook = createCodeGraphFirstHook('warn')
    await hook(makeInput('mcp__semantic-search__semantic_search'))
    const result = await hook(makeInput('Glob', { pattern: '**/*.ts' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows premature Read in warn mode (does not block)', async () => {
    const hook = createCodeGraphFirstHook('warn')
    const result = await hook(makeInput('Read', { file_path: '/proj/src/main.ts' }))
  ...
  test('allows premature Grep in warn mode (does not block)', async () => {
    const hook = createCodeGraphFirstHook('warn')
    const result = await hook(makeInput('Grep', { path: '/proj/src', pattern: 'class Foo' }))
    assert.equal(result.decision, undefined)
  })
...
describe('createCodeGraphFirstHook — block mode', () => {
  test('blocks premature Read on source files', async () => {
    ...
    const result = await hook(makeInput('Read', { file_path: '/proj/src/service.ts' }))
  ...
  test('blocks premature Grep on source paths', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Grep', { path: '/proj/src', pattern: 'function' }))
    assert.equal(result.decision, 'block')
  })
  ...
  test('allows Read after graph tool even in block mode', async () => {
    const hook = createCodeGraphFirstHook('block')
    await hook(makeInput('find_dead_code'))
    const result = await hook(makeInput('Read', { file_path: '/proj/src/service.ts' }))
    assert.deepStrictEqual(result, {})
  })
  ...
  test('allows Grep after graph tool even in block mode', async () => {
    const hook = createCodeGraphFirstHook('block')
    await hook(makeInput('mcp__code-graph__graph_map'))
    const result = await hook(makeInput('Grep', { path: '/proj/src', pattern: 'class' }))
    assert.deepStrictEqual(result, {})
  })
})
...
describe('createCodeGraphFirstHook — non-code file exemptions', () => {
  test('allows Read on package.json without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/package.json' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read on tsconfig.json without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/tsconfig.app.json' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read on .md files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/README.md' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read on .yml files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/.github/workflows/ci.yml' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read on .yaml files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/docker-compose.yaml' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read on .env files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/.env.local' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read on .json files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/data/fixtures.json' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read on .lock files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/package-lock.json' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read on .config. files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/vite.config.ts' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read on .css files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/src/styles/main.css' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read on .html files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/index.html' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Read on .svg files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/assets/logo.svg' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Grep on .json path without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Grep', { path: '/proj/config.json', pattern: 'port' }))
    assert.deepStrictEqual(result, {})
  })

  test('blocks Read on .ts files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/src/app.ts' }))
    assert.equal(result.decision, 'block')
  })

  test('blocks Read on .tsx files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/src/App.tsx' }))
    assert.equal(result.decision, 'block')
  })

  test('blocks Read on .js files without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Read', { file_path: '/proj/src/util.js' }))
    assert.equal(result.decision, 'block')
  })
})
...
describe('createCodeGraphFirstHook — enforcement window', () => {
  test('stops enforcing after 5 tool calls', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      await hook(makeInput('Bash', { command: 'echo hello' }))
    }
    // 6th call: Read should be allowed even without graph tool
    const result = await hook(makeInput('Read', { file_path: '/proj/src/deep.ts' }))
    assert.deepStrictEqual(result, {})
  })
  ...
  test('enforces within the first 5 calls', async () => {
    ...
    await hook(makeInput('Write', { file_path: '/proj/out.txt', content: 'x' }))
...
describe('createCodeGraphFirstHook — non-exploration tools passthrough', () => {
  test('allows Write without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Write', { file_path: '/proj/src/new.ts' }))
    assert.deepStrictEqual(result, {})
  })

  test('allows Bash without graph tool', async () => {
    const hook = createCodeGraphFirstHook('block')
    const result = await hook(makeInput('Bash', { command: 'npm test' }))
    assert.deepStrictEqual(result, {})
  })
