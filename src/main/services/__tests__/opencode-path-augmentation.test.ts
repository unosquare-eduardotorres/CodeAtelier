...
import { augmentOpenCodeCliPath, locateOpenCodeCli, checkOpenCodeCliSync } from '../../../shared/opencode-cli-path'

describe('OpenCode CLI PATH augmentation', () => {
  test('augmentOpenCodeCliPath adds Homebrew paths to PATH', () => runExclusive(async () => {
  ...
  test('augmentOpenCodeCliPath handles duplicate paths correctly', () => runExclusive(async () => {
    const originalPath = process.env.PATH
  ...
  test('augmentOpenCodeCliPath adds npm global bin to PATH', () => runExclusive(async () => {
    const originalPath = process.env.PATH
...
describe('OpenCode CLI spawn with PATH augmentation', () => {
  test('spawn("opencode", ...) succeeds WITH PATH augmentation if CLI is installed', () => runExclusive(async () => {
    // Augment PATH
    augmentOpenCodeCliPath()
...
describe('OpenCodeExecutor integration with PATH augmentation', () => {
  test('augmented PATH enables OpenCodeExecutor.start() to find CLI', () => runExclusive(async () => {
    ...
    try {
      ...
      if (isSpawnError) {
        assert.fail(
          `With PATH augmentation, should not get spawn error: ${err.message}`
        )
      } else {
  ...
  test('checkCliAvailable() finds CLI after augmentOpenCodeCliPath()', () => runExclusive(async () => {
    const { OpenCodeExecutor } = await import('../opencode-executor')
