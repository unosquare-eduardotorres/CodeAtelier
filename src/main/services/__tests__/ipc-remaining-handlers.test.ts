...
if (loaded.convSpec) {
  describe('conversation-specialist.ipc — channels', () => {
    const channels = [...capturedHandlers.keys()].filter(c => c.includes('convSpecialist') || c.includes('conv-specialist') || c.includes('conversationSpecialist'))
    test('registers conversation specialist channels', () => {
      assert.ok(channels.length >= 3, `Expected ≥3 channels, got ${channels.length}: ${channels.join(', ')}`)
    })
  })

  describe('conversation-specialist.ipc — validation', () => {
    // Find the list channel
    const listCh = [...capturedHandlers.keys()].find(c =>
      (c.includes('specialist') || c.includes('Specialist')) && c.includes('list') && c.includes('conv')
    )
    if (listCh) {
...
if (loaded.codeChanges) {
  describe('code-changes.ipc — channels', () => {
    const channels = [...capturedHandlers.keys()].filter(c =>
      c.includes('repo:') || c.includes('code-changes')
    )
    test('registers repo channels', () => {
      assert.ok(channels.length >= 4, `Expected ≥4 channels, got ${channels.length}: ${channels.join(', ')}`)
    })
  })

  describe('code-changes.ipc — validation', () => {
    const commitCh = [...capturedHandlers.keys()].find(c => c.includes('repo') && c.includes('commit'))
    if (commitCh) {
    ...
    const diffCh = [...capturedHandlers.keys()].find(c => c.includes('repo') && c.includes('diff'))
    if (diffCh) {
...
if (loaded.projSpec || loaded.project) {
  describe('project/specialist IPC — channels', () => {
    ...
    test('registers project channels', () => {
      assert.ok(projectChannels.length >= 2, `Expected ≥2 project channels, got ${projectChannels.length}`)
    })
  })
}
...
if (loaded.specialist) {
  describe('specialist.ipc — channels', () => {
    const channels = [...capturedHandlers.keys()].filter(c =>
      c.includes('specialist') && !c.includes('conv') && !c.includes('project')
    )
    test('registers specialist channels', () => {
      assert.ok(channels.length >= 2, `Expected ≥2 channels, got ${channels.length}: ${channels.join(', ')}`)
    })
  })
}
...
if (loaded.indexing) {
  describe('indexing.ipc — channels', () => {
    const channels = [...capturedHandlers.keys()].filter(c =>
      c.includes('index') || c.includes('code-graph') || c.includes('codeGraph')
    )
...
describe('IPC handler validation — null args rejection', () => {
  ...
  for (const ch of allChannels) {
    ...
    if (ch.includes('selectDirectory') || ch.includes('getStatuses') ||
        ch.includes('stopAll') || ch.includes('cacheEfficiency') ||
        ch.includes('getStreamingState') || ch.includes('all-statuses') ||
        ch.includes('compact') || ch.includes('stop') || ch.includes('cancel')) {
      continue
    }
  }
})
