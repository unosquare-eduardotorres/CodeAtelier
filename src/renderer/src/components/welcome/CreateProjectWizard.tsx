...
async function synthesizePlanRequirement(
  workspaceId: string,
  projectName: string,
  description: string,
...
async function finalizeToGoals(
  requirement: { text: string; degraded: boolean },
  grillDecisions: GrillDecision[]
): Promise<void> {
  useMpaStore.getState().setPreloadedGoal({
    text: requirement.text,
    autoDecompose: !requirement.degraded,
...
async function finalizeToCouncil(
  workspaceId: string,
  requirement: string,
  projectName: string
): Promise<void> {
...
async function finalizeToChat(
  ...
  try {
    ...
    await useChatStore.getState().createConversation(workspaceId, 'plan', projectName.trim())
...
// ── Shell management hook ────────────────────────────────────────────────

function useProjectShell(
  projectName: string,
  parentFolder: string,
  description: string,
  attachments: string[]
  ...
  ensureShell: () => Promise<string | null>
  discardIfNeeded: () => Promise<void>
  ...
  return { workspaceId, shellError, ensureShell, discardIfNeeded, finalizedRef }
}
...
export default function CreateProjectWizard({
  ...
  const { workspaceId, shellError, ensureShell, discardIfNeeded, finalizedRef } =
    useProjectShell(projectName, parentFolder, description, attachments)
  ...
  const handleFinalize = useCallback(
    async (destination: ProjectDestination) => {
      ...
      if (destination === 'goals') {
        const requirement = await synthesizePlanRequirement(id, projectName, description, grillDecisions, trackScores)
        await finalizeToGoals(requirement, grillDecisions)
      } else if (destination === 'council') {
        const { text } = await synthesizePlanRequirement(id, projectName, description, grillDecisions, trackScores)
        await finalizeToCouncil(id, text, projectName)
      } else if (destination === 'chat') {
        await finalizeToChat(id, projectName, description, grillDecisions, trackScores)
      }
