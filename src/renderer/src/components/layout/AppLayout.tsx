...
import { Monitor, Bot, Zap, Home, Settings, Sliders } from 'lucide-react';
...
import { useWorkspaceStore, useAgentStore, useChatStore } from '@renderer/store';
...
export default function AppLayout(): React.JSX.Element {
  ...
  const { activeConversation, createConversation } = useChatStore();
  ...
  const handleKeyDown = useCallback(
    ...
    [activeWorkspace, createConversation]
  );
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-900">
      ...
      <div
        className={`h-8 flex-shrink-0 bg-gray-900 border-b border-gray-700 flex items-center justify-between pr-4 ${isMac ? 'pl-[80px]' : 'pl-20'}`}
        style={{ WebkitAppRegion: 'drag' } as React.CSSProperties}
      >
        <span className="text-[11px] text-gray-500 font-medium">Agent Studio</span>
        ...
        <div className="flex items-center gap-1" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
          ...
          {activeWorkspace && (
            <button
              ...
              className={`p-1 rounded transition-colors ${view === 'settings' ? 'text-indigo-400 bg-gray-800' : 'text-gray-500 hover:text-gray-300 hover:bg-gray-800'}`}
          ...
          <button
            ...
            className={`p-1 rounded transition-colors ${view === 'app-settings' ? 'text-indigo-400 bg-gray-800' : 'text-gray-500 hover:text-gray-300 hover:bg-gray-800'}`}
            title="Settings"
            aria-label="Settings"
          >
            <Sliders size={13} />
      ...
      <div className="flex items-center justify-between px-4 py-1.5 bg-gray-900 border-t border-gray-700 text-[11px]">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            title="Toggle Agent Panel (Cmd+J)"
          >
            <Monitor size={12} />
