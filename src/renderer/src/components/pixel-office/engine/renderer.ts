// Adapted from pixel-agents: webview-ui/src/office/engine/renderer.ts
// Canvas 2D render loop. All rendering functions preserved.
...
/** Render agent name + status labels below each character */
export function renderAgentLabels(
  ...
  // Only show labels at zoom >= 2 to avoid clutter
  if (zoom < 2) return;
  ...
  for (const ch of characters) {
    ...
    // Skip characters mid-spawn/despawn
    if (ch.matrixEffect) continue;

    // Position below the character's feet
    const labelX = Math.round(offsetX + ch.x * zoom);
    const labelY = Math.round(offsetY + ch.y * zoom + 2 * zoom);

    // Status text
    const statusText = ch.isActive
      ? ch.state === CharacterState.TYPE ? '⌨ Working' : '📖 Reading'
      : '💤 Idle';

    const fullText = ch.displayName;

    // Background pill
    const textWidth = ctx.measureText(fullText).width;
    const statusWidth = ctx.measureText(statusText).width;
    const maxWidth = Math.max(textWidth, statusWidth);
    const pillPad = 3;
    const pillH = fontSize * 2 + 6;
    const pillX = labelX - maxWidth / 2 - pillPad;
    const pillY = labelY;
    ...
    ctx.lineTo(pillX + maxWidth + pillPad * 2 - r, pillY);
    ctx.quadraticCurveTo(pillX + maxWidth + pillPad * 2, pillY, pillX + maxWidth + pillPad * 2, pillY + r);
    ctx.lineTo(pillX + maxWidth + pillPad * 2, pillY + pillH - r);
    ctx.quadraticCurveTo(pillX + maxWidth + pillPad * 2, pillY + pillH, pillX + maxWidth + pillPad * 2 - r, pillY + pillH);
    ...
    // Status text (smaller, colored)
    const smallFont = Math.max(7, Math.round(zoom * 2.8));
    ctx.font = `${smallFont}px "Inter", system-ui, sans-serif`;
    ctx.fillStyle = ch.isActive ? '#34d399' : '#9ca3af';
    ctx.fillText(statusText, labelX, labelY + fontSize + 3);
...
export function renderFrame(
  ...
  renderAgentLabels(ctx, characters, offsetX, offsetY, zoom);
