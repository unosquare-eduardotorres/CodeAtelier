// Ported from vendor: pixel-agents/webview-ui/src/office/editor/editorState.ts
// Adapted for Agent Studio — removed VS Code dependencies, uses local constants and types.

import { DEFAULT_FLOOR_COLOR, DEFAULT_WALL_COLOR, UNDO_STACK_MAX_SIZE } from '../constants'
import type { FloorColor, OfficeLayout, TileType as TileTypeVal } from '../engine/types'
import { EditTool } from '../engine/types'

export class EditorState {
  isEditMode = false
  activeTool: EditTool = EditTool.SELECT
  selectedTileType: TileTypeVal = 1 // TileType.FLOOR_1
