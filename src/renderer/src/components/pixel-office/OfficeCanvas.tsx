  ...
  function initEngine(canvas: HTMLCanvasElement): void {
    ...
    engineRef.current = {
      ...
      removeAgent(numericId) {
        office.removeAgent(numericId)
      },
      setAgentActive(numericId, active) {
        office.setAgentActive(numericId, active)
      },
      setAgentTool(numericId, toolName) {
        office.setAgentTool(numericId, toolName)
      },
      showPermissionBubble(numericId, _text) {
        office.showPermissionBubble(numericId)
      },
      clearPermissionBubble(numericId) {
        office.clearPermissionBubble(numericId)
      },
      getTotalSeats() {
        return office.seats.size
      },
      getAgentNumericId() {
        return undefined
      }
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-3 right-3 flex items-center gap-1 bg-gray-800/80 backdrop-blur-sm rounded-lg px-1.5 py-1 border border-gray-700/50">
        ...
        <span className="text-[10px] text-gray-400 w-8 text-center font-mono">{zoomLevel}x</span>
        <button
          onClick={() => changeZoom(1)}
          className="p-1 rounded hover:bg-gray-700 text-gray-400 hover:text-gray-200 transition-colors"
          title="Zoom in"
