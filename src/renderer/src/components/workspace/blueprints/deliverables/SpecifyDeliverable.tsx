...
export function SpecifyDeliverable({
  ...
  const completion = (completionArt?.contentJson ?? spec?.contentJson) as Record<string, unknown> | undefined
  ...
  if (!spec?.contentMd) {
    return (
      <div>
        <DeliverableHeader config={config} summary="No specification artifact found" duration={duration} />
        <p className="text-xs text-text-muted italic">The specification artifact was not produced by this phase.</p>
      </div>
    )
  }

  const formattedWordCount = wordCount >= 1000 ? `${(wordCount / 1000).toFixed(1)}k` : String(wordCount)

  return (
    <div>
      <DeliverableHeader config={config} summary={`Specification drafted (${formattedWordCount} words)`} duration={duration} />
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        {checklistScore && <MetricTile label="Checklist" value={checklistScore} variant="success" />}
        ...
        <MetricTile label="Status" value={phase.status === 'complete' ? 'Complete ✓' : phase.status} variant={phase.status === 'complete' ? 'success' : 'default'} />
      </div>

      {/* Section outline */}
      {headings.length > 0 && (
        <div className="mb-6">
          <h3 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-3">Section Outline</h3>
      ...
      {/* Full specification */}
      <CappedMarkdownBlock content={spec.contentMd} label="Full Specification" maxH="max-h-[600px]" />
