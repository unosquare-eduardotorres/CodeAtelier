...
function getRecommendationDisplay(recommendation: string): {
  ...
  switch (recommendation) {
    case 'approve':
    case 'proceed':
      return { label: 'Approved — Proceed to Build', icon: CheckCircle2, bgClass: 'bg-success/10 border-success/20', textClass: 'text-success' }
    case 'fix_critical':
      return { label: 'Fix Critical Issues Before Proceeding', icon: AlertTriangle, bgClass: 'bg-warning/10 border-warning/20', textClass: 'text-warning' }
    case 're_specify':
    case 'reject':
      return { label: 'Re-specify — Significant Issues Found', icon: XCircle, bgClass: 'bg-danger/10 border-danger/20', textClass: 'text-danger' }
    default:
      return { label: recommendation, icon: CheckCircle2, bgClass: 'bg-surface-overlay border-border-subtle', textClass: 'text-text-primary' }
  }
}

// ── Findings Bar ──

function FindingsBar({ severity, count, maxCount }: { severity: string; count: number; maxCount: number }): JSX.Element {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-xs font-medium text-text-secondary w-16 text-right uppercase">{severity}</span>
      <div className="flex-1 h-5 flex items-center">
        {count > 0 && (
          <div className={`h-4 rounded-sm ${colorMap[severity] ?? 'bg-text-muted'} transition-all`} style={{ width: `${barWidth}%` }} />
        )}
      </div>
...
export function ReviewDeliverable({
  ...
  const recommendation = (json?.recommendation as string) ?? (phase.status === 'complete' ? 'approve' : 'unknown')
  ...
  // Coverage variant
  const coverageVariant = coveragePercent != null
    ? coveragePercent >= 90 ? 'success' as const : coveragePercent >= 70 ? 'warning' as const : 'danger' as const
    : 'default' as const
  ...
  if (!json && !review?.contentMd) {
    return (
      <div>
        ...
        <p className="text-xs text-text-muted italic">The review artifact was not produced by this phase.</p>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      {(() => {
        ...
        const gridCols = visibleMetrics >= 4 ? 'grid-cols-4' : visibleMetrics === 3 ? 'grid-cols-3' : 'grid-cols-2'

        return (
          <div className={`grid ${gridCols} gap-3 mb-6`}>
            {coveragePercent != null && (
              <MetricTile label="Coverage" value={`${coveragePercent}%`} variant={coverageVariant} />
            )}
            {totalRequirements > 0 && (
              <MetricTile label="Req / Tasks" value={`${requirementsWithTasks}/${totalRequirements}`} />
            )}
            <MetricTile label="Unmapped" value={`${unmappedTasks} task${unmappedTasks !== 1 ? 's' : ''}`} variant={unmappedTasks > 0 ? 'warning' : 'default'} />
            <MetricTile label="Violations" value={constitutionViolations} variant={constitutionViolations > 0 ? 'danger' : 'success'} />
          </div>
        )
      })()}
      ...
      {findings && Object.keys(findings).length > 0 && (
        <div className="mb-6">
          <h3 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-3">Findings</h3>
      ...
      {/* Review narrative */}
      {review?.contentMd && (
        <CappedMarkdownBlock content={review.contentMd} label="Review Narrative" maxH="max-h-[500px]" />
      )}
