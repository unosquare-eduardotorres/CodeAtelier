...
function humanizeRecommendation(rec: string): string {
  switch (rec) {
    ...
    case 'fix_gaps':
      return 'Fix identified gaps before shipping'
    case 'manual_review':
      return 'Manual review needed before shipping'
    default:
      return rec
  }
}
...
function getStatusDisplay(
  ...
  switch (status) {
    case 'passed':
      return {
        label: 'Verification Passed — Ready to Ship',
        icon: ShieldCheck,
    ...
    case 'gaps_found':
      return {
        label: hasRemediationTasks
          ? 'Gaps Found — Remediation Applied'
          : 'Gaps Found — Manual Review Needed',
        icon: AlertTriangle,
...
function SeverityBadge({ severity }: { severity: string }): JSX.Element {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-semibold uppercase tracking-wider border ${styles}`}
    >
      {label}
    </span>
  )
...
export function VerifyDeliverable({
  ...
  // Remediation tasks — prefer new `remediationTasks` field, fall back to legacy `tasks` R-filtered
  const rawRemediationTasks =
  ...
  const remediationTasks: RemediationTask[] = rawRemediationTasks.map((t) => ({
    taskId: String(t.taskId ?? ''),
    description: String(t.description ?? ''),
    status: t.status as string | undefined
  }))

  // Findings — issues identified by the verifier
  const rawFindings = (json?.findings as Array<Record<string, unknown>>) ?? []
  ...
  if (!json && verify?.contentMd) {
    return (
      <div>
        <DeliverableHeader
          config={config}
          summary="Verification report (structured status unavailable)"
          duration={duration}
        />
    ...
    return (
      <div>
        ...
        <p className="text-xs text-text-muted italic">
          The verification artifact was not produced by this phase.
        </p>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      {/* 3. Findings — the primary "what went wrong" answer */}
      {findings.length > 0 && (
        <div className="mb-6">
          ...
          <div className="space-y-3">
            {findings.map((finding, i) => (
              <div
                key={i}
                className="rounded-xl border border-border-subtle bg-surface-overlay p-4"
              >
              ...
              <tbody>
                {remediationTasks.map((task) => (
                  <tr key={task.taskId} className="border-b border-border-subtle last:border-b-0">
                    <td className="px-4 py-2 text-xs font-mono text-text-muted w-16">
                      {task.taskId}
                    </td>
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {humanVerification.map((item, i) => (
              <div
                key={i}
                className="flex items-start gap-2 px-3 py-2 rounded-lg bg-info/5 border border-info/10"
              >
      ...
      {/* 7. Artifact verification / Key links / Coverage / Anti-patterns */}
      {artifacts && (
        <div className="mb-6">
          <h3 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-3">
      ...
      {keyLinks && (
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <MetricTile
              label="Broken"
              value={keyLinks.broken ?? 0}
              variant={(keyLinks.broken ?? 0) > 0 ? 'danger' : 'success'}
            />
          </div>
        </div>
      )}
      ...
      {/* 8. Full Verification Report — collapsed by default */}
      {verify?.contentMd && (
        <details className="mt-6 group">
          <summary className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-text-muted uppercase tracking-wider mb-3 select-none list-none [&::-webkit-details-marker]:hidden">
            <ChevronDown size={12} className="transition-transform group-open:rotate-180" />
            Full Verification Report
          </summary>
          <CappedMarkdownBlock
            content={verify.contentMd}
